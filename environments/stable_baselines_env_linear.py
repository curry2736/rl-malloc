...
class StableBaselineEnvLinear(BaseEnv):
    def __init__(self, history_len=0, page_size=50, allocator="ff_bad"):
        ...
        self.observation_space = gym.spaces.Box(low=0, high=page_size+1, shape=(1, self.feature_len + 1), dtype=np.float32)
    ...
    def _get_state(self, rq):
        ...
        if len(self.page.free_list) > 0:
            ...
            avg_free_idx = self.page_size+1
            avg_free_block_size = self.page_size+1
        ...
        if len(self.page.allocated_list) > 0:
            ...
            avg_alloc_block_size = self.page_size+1
            avg_alloc_idx = self.page_size+1
        ...
        return np.concatenate([features, rq], axis=0)[None, :]
